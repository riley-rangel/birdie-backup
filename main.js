...
function renderDetails(courseData) {
  ...
  var $addressSection1 = document.createElement('div')
  var $addressSection2 = document.createElement('div')
  ...
  var $highlightTitle1 = document.createElement('h5')
  var $highlightTitle2 = document.createElement('h5')
  var $highlightTitle3 = document.createElement('h5')
  ...
  $addressCard.setAttribute('class', 'row address-card z-depth-2')
  $addressSection1.setAttribute('class', 'col s8')
  $addressSection2.setAttribute('class', 'col s4')
  ...
  $addressCard.append($addressSection1, $addressSection2)
  $addressSection1.append($name, $address, $address2)
  $addressSection2.appendChild($start)
...
function renderRecap(roundData) {
  ...
  $highlightTitle3.setAttribute('class', 'highlight-title')
  $highlightTitle3.textContent = 'Best Hole'
  $highlight1.setAttribute('class', 'highlight z-depth-2')
  $highlight1.textContent = timeElapsed(roundData.startTime, roundData.endTime)
  $highlight2.setAttribute('class', 'highlight z-depth-2')
  $highlight2.textContent = calcHandicap(roundData)
  $highlight3.setAttribute('class', 'highlight z-depth-2')
...
function timeElapsed(startTime, endTime) {
  var hour = endTime.getHours() - startTime.getHours()
  if (hour < 10) {
    hour = '0' + hour
  }
  var min = endTime.getMinutes() - startTime.getMinutes()
  if (min < 10) {
    min = '0' + min
  }
...
function calcHandicap(roundData) {
  var course = courses[roundData.courseId - 1]
  var score = calcScorecard(roundData.playerScore, 1, 18)
  var courseRating = course.usgaRating
  var slope = course.slope
  var handicap = Math.round(((score - courseRating) * 113 / slope) * 10) / 10
  return handicap
}

function bestHole(roundData) {
  var coursePar = roundData.parIndex
  var playerScore = roundData.playerScore
  var overUnder = []
  var low = 0
  for (var i = 0; i < playerScore.length; i++) {
    var net = playerScore[i] - coursePar[i]
    overUnder.push(net)
  }
  for (var n = 0; n < overUnder.length; n++) {
    var num = overUnder[n]
    if (num < low) {
      low = num
    }
  }
  var best = overUnder.indexOf(low) + 1
  return best
}
